...
describe("hydrogen package assets", () => {
  const root = path.join(__dirname, "..");

  it("provides keymaps and menus as JSON, not CSON", () => {
    expect(fs.existsSync(path.join(root, "keymaps/hydrogen.json"))).toBe(true);
    expect(fs.existsSync(path.join(root, "menus/hydrogen.json"))).toBe(true);
    expect(fs.existsSync(path.join(root, "keymaps/hydrogen.cson"))).toBe(false);
    expect(fs.existsSync(path.join(root, "menus/hydrogen.cson"))).toBe(false);
  });

  it("ships keymap and menu JSON that parse", () => {
    const keymap = JSON.parse(fs.readFileSync(path.join(root, "keymaps/hydrogen.json"), "utf8"));
    expect(keymap["atom-workspace"]).toBeDefined();

    const menu = JSON.parse(fs.readFileSync(path.join(root, "menus/hydrogen.json"), "utf8"));
  ...
  it("ships a CSS stylesheet built on custom properties, not Less", () => {
    expect(fs.existsSync(path.join(root, "styles/hydrogen.css"))).toBe(true);
    expect(fs.existsSync(path.join(root, "styles/hydrogen.less"))).toBe(false);

    const css = fs.readFileSync(path.join(root, "styles/hydrogen.css"), "utf8");
