...
/*
 * The `joveKernel` class wraps Jove's internal representation of kernels
 * and exposes a small set of methods that should be usable by plugins.
 * @class JoveKernel
 */

export default class JoveKernel {
  constructor(_kernel) {
   ...
   * This gives access to raw Jove-internal result format.
   *
   * @param {String} code - The code to execute
  ...
  _assertNotDestroyed() {
    ...
    if (this.destroyed) {
      throw new Error("joveKernel: operation not allowed because the kernel has been destroyed");
    }
  }
   ...
   *
   * @param {JoveKernelMiddleware} middleware
   */
