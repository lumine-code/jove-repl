...
export default class ZMQKernel extends KernelTransport {
  ...
  constructor(kernelSpec, grammar, options, onStarted) {
    ...
    // Otherwise spawnteract deletes the file and jove-repl's restart kernel fails
  ...
  monitorNotifications(childProcess) {
    childProcess.stdout.on("data", (data) => {
      ...
      if (atom.config.get("jove-repl.kernelNotifications")) {
  ...
  _executeStartupCode() {
    ...
    if (this.language === "python") {
      const autoreloadMode = atom.config.get("jove-repl.pythonAutoreload");
      if (autoreloadMode !== "off") {
        log("KernelManager: Loading Python autoreload extension");
        this.execute("%load_ext autoreload\n", () => {});
        const printActivity = atom.config.get("jove-repl.pythonAutoreloadPrint");
