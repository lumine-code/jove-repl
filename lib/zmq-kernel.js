...
import { launchSpec, launchSpecFromConnectionInfo } from "./kernel-launcher";
import Config from "./config";
import KernelTransport from "./kernel-transport";
import { log, js_idx_to_char_idx } from "./utils";
import { Message, Socket } from "./jmp";
import { msgSpecToNotebookFormat, OUTPUT_TYPES } from "./output-utils";
...
export default class ZMQKernel extends KernelTransport {
  ...
  monitorNotifications(childProcess) {
    childProcess.stdout.on("data", (data) => {
      ...
      if (atom.config.get("jupyter-repl.kernelNotifications")) {
    ...
    childProcess.stderr.on("data", (data) => {
      ...
      switch (_ipythonLogLevel(text)) {
        case "WARNING":
          atom.notifications.addWarning(this.kernelSpec.display_name, {
            description: text,
            dismissable: true,
          });
          break;
    ...
    childProcess.on("exit", (code, signal) => {
      ...
      if (store.startingKernels.has(this.kernelSpec.display_name)) {
        ...
        atom.notifications.addError(`${this.kernelSpec.display_name}: kernel process died`, {
          description:
  ...
  monitor(done, prev) {
    try {
      ...
      const monitor = (socketName, socket) => {
        ...
        // For initial connection only: if socket is already connected, proceed immediately
        // During restart (prev=true), we MUST wait for reconnection to the NEW kernel
        if (!prev && socket.isConnected) {
          log(`ZMQKernel: ${socketName} already connected (initial)`);
          onConnect({ socketName, socket });
          return;
        }
  ...
  _executeStartupCode() {
    ...
    if (this.language === "python") {
      const autoreloadMode = atom.config.get("jupyter-repl.pythonAutoreload");
      if (autoreloadMode !== "off") {
        log("KernelManager: Loading Python autoreload extension");
        this.execute("%load_ext autoreload\n", () => {});
        const printActivity = atom.config.get("jupyter-repl.pythonAutoreloadPrint");
