...
import { SelectListView, highlightMatches, createTwoLineItem } from "@lumine-code/select-list";
...
export default class ExecPanel {
  constructor(store) {
    ...
    this.selectList = new SelectListView({
      className: "jove-repl exec-panel",
      ...
      elementForItem: (entry, { filterKey, matchIndices }) => {
        // Primary: code text with optional highlighting
        const primary =
          this._currentQuery && matchIndices
            ? highlightMatches(filterKey, matchIndices)
            : entry.code;
  ...
  async execute() {
    ...
    const joveKernel = kernel.getPluginWrapper();
    const result = await joveKernel.execute(code);
