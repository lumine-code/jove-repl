...
// "2 min. ago" reads better than a clock time on a list that is always about
// the last few minutes of work. `numeric: "auto"` is what turns the near end
// of the scale into "now" and "yesterday" rather than counting them out.
const RELATIVE_TIME = new Intl.RelativeTimeFormat(undefined, {
  numeric: "auto",
  style: "narrow",
});

// Each unit with the count of itself that makes the next one up.
...
/**
 * Formats a past timestamp as how long ago it was, in the largest unit that
 * still counts more than one of itself.
 * @param {Date} timestamp - The moment to describe
 * @returns {string} The localized relative time
 */
export function relativeTime(timestamp) {
  let value = (timestamp.getTime() - Date.now()) / 1000;
  ...
  constructor(store) {
    ...
    this.selectList = atom.workspace.buildSelectList({
      ...
      elementForItem: (entry, { filterKey, highlight }) => ({
        ...
        trailing: [
          { text: relativeTime(entry.timestamp), className: "exec-time badge" },
          statusBadge(entry),
        ],
      }),
