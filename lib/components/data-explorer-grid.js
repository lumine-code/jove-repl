...
class CanvasGrid extends React.Component {
  ...
  componentDidMount() {
    ...
    this._commands = atom.commands.add(this.wrapRef.current, {
      ...
      "jove-repl:data-grid-page-up": page(-1),
      "jove-repl:data-grid-page-down": page(1),
      "jove-repl:data-grid-select-page-up": page(-1, true),
      "jove-repl:data-grid-select-page-down": page(1, true),
      "jove-repl:data-grid-move-to-row-start": moveTo(null, 0),
      "jove-repl:data-grid-move-to-row-end": command(() => {
        const { nCols } = this.gridSize();
        this.moveActiveSelectionTo(null, nCols - 1);
      }),
      "jove-repl:data-grid-select-to-row-start": moveTo(null, 0, true),
      "jove-repl:data-grid-select-to-row-end": command(() => {
        const { nCols } = this.gridSize();
        this.moveActiveSelectionTo(null, nCols - 1, true);
      }),
      "jove-repl:data-grid-select-row": command(() => this.selectActiveRow()),
      "jove-repl:data-grid-select-column": command(() => this.selectActiveColumn()),
