...
class CanvasGrid extends React.Component {
  ...
  componentDidMount() {
    ...
    this._commands = atom.commands.add(this.wrapRef.current, {
      ...
      "hydrogen:data-grid-page-up": page(-1),
      "hydrogen:data-grid-page-down": page(1),
      "hydrogen:data-grid-select-page-up": page(-1, true),
      "hydrogen:data-grid-select-page-down": page(1, true),
      "hydrogen:data-grid-move-to-row-start": moveTo(null, 0),
      "hydrogen:data-grid-move-to-row-end": command(() => {
        const { nCols } = this.gridSize();
        this.moveActiveSelectionTo(null, nCols - 1);
      }),
      "hydrogen:data-grid-select-to-row-start": moveTo(null, 0, true),
      "hydrogen:data-grid-select-to-row-end": command(() => {
        const { nCols } = this.gridSize();
        this.moveActiveSelectionTo(null, nCols - 1, true);
      }),
      "hydrogen:data-grid-select-row": command(() => this.selectActiveRow()),
      "hydrogen:data-grid-select-column": command(() => this.selectActiveColumn()),
