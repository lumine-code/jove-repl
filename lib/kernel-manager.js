...
export class KernelManager {
        ...
        if (!kernelSpec) {
          ...
          const pythonDescription =
            grammar && /python/g.test(grammar.scopeName)
              ...
              : "";
          const description = `Check that the language for this file is set in Lumine, that you have a Jupyter kernel installed for it, and that you have configured the language mapping in Hydrogen preferences.${pythonDescription}`;
  ...
  async getKernelSpecForGrammar(grammar, editor) {
    ...
    if (kernelSpecs.length === 1 && atom.config.get("hydrogen.autoKernelPicker")) {
