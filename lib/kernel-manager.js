...
export class KernelManager {
        ...
        if (!kernelSpec) {
          ...
          const pythonDescription =
            grammar && /python/g.test(grammar.scopeName)
              ...
              : "";
          const description = `Check that the language for this file is set in Lumine, that you have a Jupyter kernel installed for it, and that you have configured the language mapping in Jove preferences.${pythonDescription}`;
  ...
  startKernel(kernelSpec, grammar, editor, filePath, onStarted) {
    ...
    switch (atom.config.get("jove-repl.startDir")) {
  ...
  async getKernelSpecForGrammar(grammar, editor) {
    ...
    if (kernelSpecs.length === 1 && atom.config.get("jove-repl.autoKernelPicker")) {
  ...
  async updateKernelSpecs(grammar, silent) {
    ...
    if (!silent) {
      if (kernelSpecs.length === 0) {
          ...
          buttons: [
            {
              ...
              onDidClick: () =>
                shell.openExternal("https://nteract.gitbooks.io/jove-repl/docs/Installation.html"),
            },
            {
        ...
        const message = "jove-repl kernels updated:";
