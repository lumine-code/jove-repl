...
export function provideAutocomplete(store) {
  const autocompleteProvider = {
    enabled: atom.config.get("jupyter-repl.autocomplete"),
    ...
    suggestionPriority: atom.config.get("jupyter-repl.autocompleteSuggestionPriority"),
    // It won't suppress providers with lower priority.
    excludeLowerPriority: false,
    suggestionDetailsEnabled: atom.config.get("jupyter-repl.showInspectorResultsInAutocomplete"),
    ...
    getSuggestions({ editor, bufferPosition, prefix }) {
      ...
      let minimumWordLength = atom.config.get("autocomplete.minimumWordLength");
  ...
  store.subscriptions.add(
    atom.config.observe("jupyter-repl.autocomplete", (v) => {
      autocompleteProvider.enabled = v;
    }),
    atom.config.observe("jupyter-repl.autocompleteSuggestionPriority", (v) => {
      autocompleteProvider.suggestionPriority = v;
    }),
    atom.config.observe("jupyter-repl.showInspectorResultsInAutocomplete", (v) => {
