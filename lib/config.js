...
const Config = {
  ...
  getGateways(_default = []) {
    ...
    try {
      ...
      atom.notifications.addError("Your Hydrogen gateways config is broken", {
  ...
  ensureGatewaysFile(gatewaysPath) {
    ...
    let contents = "[]\n";
    const oldValue = atom.config.get("hydrogen.gateways");
    ...
    if (oldValue && typeof oldValue === "string") {
      try {
        ...
        atom.notifications.addWarning("Could not migrate Hydrogen gateways setting", {
          detail: error.message || String(error),
          dismissable: true,
        });
      }
    ...
    if (migratedOldSetting) {
      atom.config.unset("hydrogen.gateways");
    }
  },
  ...
  schema: {
    ...
    showInspectorResultsInAutocomplete: {
      ...
      description:
        "If enabled, hydrogen will try to show the results from kernel inspection in each autocomplete suggestion's description. May slow down the autocompletion performance. **Note**: Even if you disable this, you would still get autocomplete suggestions.",
      type: "boolean",
      default: false,
    },
    ...
    importNotebookURI: {
      ...
      description:
        "If enabled, opening a file with extension `.ipynb` will import the notebook file's source into a new tab. If disabled, or if the hydrogen package is not activated, the raw file will open in Lumine as normal.",
      type: "boolean",
      default: true,
    },
