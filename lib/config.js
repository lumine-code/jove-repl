...
const Config = {
  getJson(key, _default = {}) {
    ...
    const value = atom.config.get(`jupyter-repl.${key}`);
  ...
  getGateways(_default = []) {
    ...
    try {
      ...
      atom.notifications.addError("Your Jupyter gateways config is broken", {
  ...
  ensureGatewaysFile(gatewaysPath) {
    ...
    let contents = "[]\n";
    const oldValue = atom.config.get("jupyter-repl.gateways");
    ...
    if (oldValue && typeof oldValue === "string") {
      try {
        ...
        atom.notifications.addWarning("Could not migrate Jupyter gateways setting", {
          detail: error.message || String(error),
          dismissable: true,
        });
      }
    ...
    if (migratedOldSetting) {
      atom.config.unset("jupyter-repl.gateways");
    }
  },
};
