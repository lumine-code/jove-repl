...
const Config = {
  getJson(key, _default = {}) {
    ...
    try {
      ...
      const message = `Your Hydrogen-next config is broken: ${key}`;
  ...
  getGateways(_default = []) {
    ...
    try {
      ...
      atom.notifications.addError("Your Hydrogen-next gateways config is broken", {
  ...
  ensureGatewaysFile(gatewaysPath) {
    ...
    let contents = "[]\n";
    const oldValue = atom.config.get("hydrogen-next.gateways");
    ...
    if (oldValue && typeof oldValue === "string") {
      try {
        ...
        atom.notifications.addWarning("Could not migrate Hydrogen-next gateways setting", {
          detail: error.message || String(error),
          dismissable: true,
        });
      }
    ...
    if (migratedOldSetting) {
      atom.config.unset("hydrogen-next.gateways");
    }
  },
  ...
  schema: {
    ...
    showInspectorResultsInAutocomplete: {
      ...
      description:
        "If enabled, hydrogen-next will try to show the results from kernel inspection in each autocomplete suggestion's description. May slow down the autocompletion performance. **Note**: Even if you disable this, you would still get autocomplete suggestions.",
      type: "boolean",
      default: false,
    },
    ...
    importNotebookURI: {
      ...
      description:
        "If enabled, opening a file with extension `.ipynb` will import the notebook file's source into a new tab. If disabled, or if the hydrogen-next package is not activated, the raw file will open in Pulsar as normal.",
      type: "boolean",
      default: true,
    },
    ...
    statusBarDisable: {
      order: 6,
      title: "Disable the hydrogen-next status bar",
      description: "If enabled, no kernel information will be provided in Pulsar's status bar.",
      type: "boolean",
      default: false,
    },
    statusBarKernelInfo: {
      order: 7,
      title: "Detailed kernel information in the hydrogen-next status bar",
      description:
        "If enabled, more detailed kernel information (execution count, execution time if available) will be shown in the hydrogen-next status bar. This requires the above **Disable the hydrogen-next status bar** setting to be `false` to work.",
      type: "boolean",
      default: true,
    },
    autoKernelPicker: {
    ...
    autoScroll: {
      ...
      description:
        "If enabled, hydrogen-next will automatically scroll to the bottom of the result view.",
      type: "boolean",
      default: true,
    },
    scrollOnMoveDown: {
    ...
    wrapOutput: {
      ...
      description:
        "If enabled, your output code from hydrogen-next will break long text and items.",
      type: "boolean",
      default: true,
    },
    outputAreaDefault: {
