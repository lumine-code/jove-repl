...
import { log, msgSpecToNotebookFormat, formatElapsedTime } from "./utils";
...
import JoveKernel from "./plugin-api/jove-kernel";
...
// that JoveKernelMiddleware objects are mutable: they may lose/gain methods
...
class MiddlewareAdapter {
  ...
  // implementing JoveKernelMiddlewareThunk. Once multiple plugin API
  // versions exist, we may want to generate a JoveKernelMiddlewareThunk
...
export default class Kernel {
  ...
  getPluginWrapper() {
    if (!this.pluginWrapper) {
      this.pluginWrapper = new JoveKernel(this);
    }
  ...
  /*
   * Takes a callback that accepts execution results in a jove-repl-internal
   ...
   * protocol in order to maximize compatibility, but jove-repl internally uses
   * its own types.
   */
  ...
  destroy() {
    ...
    try {
      // The Data Explorer is a singleton fed by a kernel; clear it if it was
      // showing this one.
      if (dataExplorerStore.kernel === this) {
        dataExplorerStore.reset();
      }
    } catch (e) {
      log("Kernel: Error resetting dataExplorerStore:", e);
    }

    try {
      if (inspectorStore.kernel === this) {
        inspectorStore.reset();
      }
    } catch (e) {
      log("Kernel: Error resetting inspectorStore:", e);
    }
