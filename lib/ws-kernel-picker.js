...
class CustomListView {
  ...
  constructor() {
    this.selectList = new SelectListView({
      className: "jove-repl ws-kernel-picker",
...
export default class WSKernelPicker {
  ...
  async toggle(_kernelSpecFilter) {
    ...
    if (!gateways.length) {
      atom.notifications.addError("No remote kernel gateways available", {
        description:
          "Use Jove: Open Gateways Config to edit gateways.json. Jove can use remote kernels on either a Jupyter Kernel Gateway or Jupyter notebook server.",
      });
      return;
    }
  ...
  showGatewayConnectionError(error, gatewayOptions) {
    ...
    atom.notifications.addError("Connection to gateway failed", {
      description: "Jove could not load kernel specs from the selected gateway.",
      detail: `Gateway: ${gatewayOptions.baseUrl}\nError: ${errorMessage}`,
      dismissable: true,
    });
  }
