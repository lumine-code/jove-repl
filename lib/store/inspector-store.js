...
function buildPythonResultInspectorCode(expression, targetName) {
  return `
def _jupyter_repl_inspector_eval():
    import ast
...
_jupyter_repl_inspector_eval()
del _jupyter_repl_inspector_eval
`;
}
...
class InspectorStore {
  ...
  _fetchPythonExpressionResult(requestId, expression) {
    const targetName = `__jupyter_repl_inspector_result_${requestId}`;
