...
const DATA_EXPLORER_URI = "atom://jove-repl/data-explorer";
const INSPECTOR_URI = "atom://jove-repl/inspector";
...
export class Store {
  ...
  globalMode = Boolean(atom.config.get("jove-repl.globalMode"));
  // Allow external packages (like jove-view) to set the current kernel directly
  ...
  get kernel() {
    ...
    // External kernel takes priority (set by jove-view or other packages).
  ...
  dispose() {
    ...
    this.runningKernels.forEach((kernel) => {
      try {
        ...
        console.error("[jove-repl] Error destroying kernel:", e);
      }
    });
    this.runningKernels = [];
   ...
   * Used by jove-view to make its kernels visible to Variable Explorer, etc.
