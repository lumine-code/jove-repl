...
class Store {
  ...
  constructor() {
    ...
    // the two panels that report their own. Every one of those is announced by
    ...
    //
    // Held apart from `subscriptions`, which deactivation disposes: the store
    // outlives that, and the constructor does not run again to re-add these.
    dataExplorerStore.onDidUpdate(() => this._notifyKernelChanged());
    inspectorStore.onDidUpdate(() => this._notifyKernelChanged());
  }
