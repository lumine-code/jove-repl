...
/**
 * Adds/removes the `hydrogen-kernel` class on every open text editor whose file
...
function updateEditorKernelClasses() {
  ...
  for (const editor of atom.workspace.getTextEditors()) {
    ...
    element.classList.toggle("hydrogen-kernel", liveFiles.has(filePath));
  }
}
...
export function activate() {
  ...
  store.subscriptions.add(
    atom.config.onDidChange("hydrogen.languageMappings", ({ oldValue }) => {
  ...
  store.subscriptions.add(
    atom.commands.add("atom-text-editor:not([mini])", {
      "hydrogen:run": (event) => run(false, event),
      "hydrogen:run-all": (event) => runAll(null, event),
      "hydrogen:run-all-above": (event) => runAllAbove(event),
      "hydrogen:run-and-move-down": (event) => run(true, event),
      "hydrogen:run-cell": (event) => runCell(false, event),
      "hydrogen:run-cell-and-move-down": (event) => runCell(true, event),
      "hydrogen:toggle-watches": () => atom.workspace.toggle(WATCHES_URI),
      "hydrogen:toggle-variable-explorer": () => atom.workspace.toggle(VARIABLE_EXPLORER_URI),
      "hydrogen:toggle-output-area": () => require("./commands").toggleOutputMode(),
      "hydrogen:start-local-kernel": (event) => startZMQKernel(event),
      "hydrogen:connect-to-remote-kernel": () => connectToWSKernel(),
      "hydrogen:connect-to-existing-kernel": () => connectToExistingKernel(),
      "hydrogen:add-watch": () => addWatch(),
      "hydrogen:remove-watch": (event) => removeWatch(event),
      "hydrogen:update-kernels": () => updateKernels(),
      "hydrogen:interrupt-kernel": (event) => handleKernelSignal("interrupt-kernel", event),
      "hydrogen:restart-kernel": (event) => handleKernelSignal("restart-kernel", event),
      "hydrogen:shutdown-kernel": (event) => handleKernelSignal("shutdown-kernel", event),
      "hydrogen:rename-remote-session": () =>
        handleKernelCommand({ command: "rename-kernel" }, store),
      "hydrogen:disconnect-remote-session": () =>
        handleKernelCommand({ command: "disconnect-kernel" }, store),
      "hydrogen:export-notebook": () => require("./export-notebook").exportNotebook(),
      "hydrogen:fold-current-cell": () => foldCurrentCell(),
      "hydrogen:fold-all-but-current-cell": () => foldAllButCurrentCell(),
      "hydrogen:clear-results": (event) => clearResults(event),
      ...
      "hydrogen:run-all-inline": (event) => runAllInline(event),
      "hydrogen:recalculate-all-inline": () => recalculateAllInline(),
      "hydrogen:run-all-above-inline": (event) => runAllAboveInline(event),
      "hydrogen:run-all-below-inline": (event) => runAllBelowInline(event),
      "hydrogen:recalculate-all-above-inline": () => recalculateAllAboveInline(),
      "hydrogen:go-to-next-cell": () => require("./cell-navi").nextCell(),
      "hydrogen:go-to-previous-cell": () => require("./cell-navi").previousCell(),
      "hydrogen:select-cell": () => require("./cell-navi").selectCell(),
      "hydrogen:select-previous-cell": () => require("./cell-navi").selectUp(),
      "hydrogen:select-next-cell": () => require("./cell-navi").selectDown(),
      "hydrogen:move-cell-up": () => require("./cell-navi").moveCellUp(),
      "hydrogen:move-cell-down": () => require("./cell-navi").moveCellDown(),
      "hydrogen:open-jupyter-console": () =>
        require("./launch-jupyter").openJupyterConsole(terminalService),
      "hydrogen:spawn-jupyter-console": () =>
        require("./launch-jupyter").spawnJupyterConsole(terminalSpawnService),
      "hydrogen:copy-jupyter-console-command": () =>
        require("./launch-jupyter").copyJupyterConsoleCommand(),
    }),
    atom.commands.add(".jupyter-notebook", {
      "hydrogen:run": (event) => runAdapterCommand("editor", false, event),
      "hydrogen:run-all": (event) => runAdapterCommand("all", false, event),
      "hydrogen:run-all-above": (event) => runAdapterCommand("above", false, event),
      "hydrogen:run-and-move-down": (event) => runAdapterCommand("editor", true, event),
      "hydrogen:run-cell": (event) => runAdapterCommand("active", false, event),
      "hydrogen:run-cell-and-move-down": (event) => runAdapterCommand("active", true, event),
      "hydrogen:run-all-inline": (event) => runAdapterCommand("all", false, event),
      "hydrogen:run-all-above-inline": (event) => runAdapterCommand("above", false, event),
      "hydrogen:run-all-below-inline": (event) => runAdapterCommand("below", false, event),
      "hydrogen:clear-results": (event) => clearAdapterResults(event),
      "hydrogen:start-local-kernel": (event) => startAdapterLocalKernel(event),
      "hydrogen:interrupt-kernel": (event) => handleAdapterKernelSignal("interrupt-kernel", event),
      "hydrogen:restart-kernel": (event) => handleAdapterKernelSignal("restart-kernel", event),
      "hydrogen:shutdown-kernel": (event) => handleAdapterKernelSignal("shutdown-kernel", event),
    }),
    atom.commands.add("atom-workspace", {
      "hydrogen:import-notebook": (event) => require("./import-notebook").importNotebook(event),
      "hydrogen:open-data-explorer": () => exploreData(),
      "hydrogen:debug-toggle": () => debugToggle(),
      "hydrogen:toggle-kernel-monitor-focus": () => toggleKernelMonitorFocus(),
      "hydrogen:toggle-exec-panel": () => toggleExecPanel(),
      "hydrogen:open-examples": () => openExamples(),
      "hydrogen:open-gateways": () => Config.openGateways(),
      "hydrogen:shutdown-all-kernels": () => shutdownAllKernels(),
      "hydrogen:toggle-inspector-focus": () =>
        require("./commands").showInspector(store, hydrogenAdapterServices),
      "hydrogen:inspect-under-cursor": () =>
        require("./commands").inspectUnderCursor(store, hydrogenAdapterServices),
      "hydrogen:attach-to-claude": () => attachResultToClaude(),
    }),
  );
  ...
  store.subscriptions.add(
    atom.workspace.observeTextEditors((editor) => {
      ...
      // Apply the `hydrogen-kernel` class to this editor in case its file already
      ...
      if (atom.config.get("hydrogen.cellMarkers")) {
  ...
  // Keep the `hydrogen-kernel` editor class in sync as kernels start and stop.
