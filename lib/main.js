...
export function activate() {
  ...
  store.subscriptions.add(
    atom.config.observe("jupyter-repl.statusBarDisable", (newValue) => {
      store.setConfigValue("jupyter-repl.statusBarDisable", Boolean(newValue));
    }),
    atom.config.observe("jupyter-repl.statusBarKernelInfo", (newValue) => {
      store.setConfigValue("jupyter-repl.statusBarKernelInfo", Boolean(newValue));
    }),
  );
  store.subscriptions.add(
    atom.commands.add("atom-text-editor:not([mini])", {
      ...
      "jupyter-repl:toggle-watches": () => atom.workspace.toggle(WATCHES_URI),
      "jupyter-repl:toggle-variable-explorer": () => atom.workspace.toggle(VARIABLE_EXPLORER_URI),
      "jupyter-repl:toggle-output-area": () => require("./commands").toggleOutputMode(),
      "jupyter-repl:start-local-kernel": (event) => startZMQKernel(event),
      "jupyter-repl:connect-to-remote-kernel": () => connectToWSKernel(),
      "jupyter-repl:connect-to-existing-kernel": () => connectToExistingKernel(),
      "jupyter-repl:add-watch": () => addWatch(),
      "jupyter-repl:remove-watch": (event) => removeWatch(event),
      "jupyter-repl:update-kernels": () => updateKernels(),
      "jupyter-repl:interrupt-kernel": (event) => handleKernelSignal("interrupt-kernel", event),
      "jupyter-repl:restart-kernel": (event) => handleKernelSignal("restart-kernel", event),
      "jupyter-repl:shutdown-kernel": (event) => handleKernelSignal("shutdown-kernel", event),
      ...
      "jupyter-repl:clear-results": (event) => clearResults(event),
      ...
      "jupyter-repl:open-jupyter-console": () =>
        require("./launch-jupyter").openJupyterConsole(terminalService),
      "jupyter-repl:spawn-jupyter-console": () =>
        require("./launch-jupyter").spawnJupyterConsole(terminalSpawnService),
      "jupyter-repl:copy-jupyter-console-command": () =>
        require("./launch-jupyter").copyJupyterConsoleCommand(),
    }),
    atom.commands.add(".jupyter-notebook", {
      "jupyter-repl:run": (event) => runAdapterCommand("editor", false, event),
      "jupyter-repl:run-all": (event) => runAdapterCommand("all", false, event),
      "jupyter-repl:run-all-above": (event) => runAdapterCommand("above", false, event),
      "jupyter-repl:run-and-move-down": (event) => runAdapterCommand("editor", true, event),
      "jupyter-repl:run-cell": (event) => runAdapterCommand("active", false, event),
      "jupyter-repl:run-cell-and-move-down": (event) => runAdapterCommand("active", true, event),
      "jupyter-repl:run-all-inline": (event) => runAdapterCommand("all", false, event),
      "jupyter-repl:run-all-above-inline": (event) => runAdapterCommand("above", false, event),
      "jupyter-repl:run-all-below-inline": (event) => runAdapterCommand("below", false, event),
      "jupyter-repl:clear-results": (event) => clearAdapterResults(event),
      "jupyter-repl:start-local-kernel": (event) => startAdapterLocalKernel(event),
      "jupyter-repl:interrupt-kernel": (event) =>
        handleAdapterKernelSignal("interrupt-kernel", event),
      "jupyter-repl:restart-kernel": (event) => handleAdapterKernelSignal("restart-kernel", event),
      "jupyter-repl:shutdown-kernel": (event) =>
    ...
    atom.commands.add("atom-workspace", {
      ...
      "jupyter-repl:open-data-explorer": () => exploreData(),
      "jupyter-repl:debug-toggle": () => debugToggle(),
      "jupyter-repl:toggle-kernel-monitor-focus": () => toggleKernelMonitorFocus(),
      "jupyter-repl:toggle-exec-panel": () => toggleExecPanel(),
      "jupyter-repl:open-examples": () => openExamples(),
      "jupyter-repl:open-gateways": () => Config.openGateways(),
      "jupyter-repl:shutdown-all-kernels": () => shutdownAllKernels(),
      "jupyter-repl:toggle-inspector-focus": () =>
        require("./commands").showInspector(store, jupyterAdapterServices),
      "jupyter-repl:inspect-under-cursor": () =>
        require("./commands").inspectUnderCursor(store, jupyterAdapterServices),
...
export function provideJupyterKernel() {
  if (!jupyterProvider) {
...
export function provideJupyterBreakpoints() {
...
export function consumeJupyterAdapter(service) {
...
function connectToExistingKernel() {
  // `@lumine-code/babel-preset` runs `add-module-exports`, so a module whose
  // only export is `default` becomes the value itself — there is no `.default`
  // property to reach through.
  const toggleExistingKernelPicker = require("./existing-kernel-picker");
  toggleExistingKernelPicker();
}
...
async function exploreData() {
  ...
  // for embedded editors (e.g. jupyter-view notebook cells), which are not the
