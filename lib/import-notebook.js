...
function ipynbOpener(uri) {
  if (
    ...
    atom.config.get("jupyter-repl.importNotebookURI") !== true
  ) {
    return undefined;
  }
  ...
  if (atom.packages.getLoadedPackage("jupyter-view")) {
    return undefined;
  }
  return _loadNotebook(uri, atom.config.get("jupyter-repl.importNotebookResults"));
}
...
async function importNotebook(event) {
  ...
  // registered on `atom-workspace`, so `event.target` is the element that was
  ...
  if (filenameFromTreeView && path.extname(filenameFromTreeView) === ".ipynb") {
    return _loadNotebook(
      ...
      atom.config.get("jupyter-repl.importNotebookResults"),
    );
  }
  ...
  for (const filePath of filePaths) {
    ...
    _loadNotebook(filePath, atom.config.get("jupyter-repl.importNotebookResults"));
  }
}
