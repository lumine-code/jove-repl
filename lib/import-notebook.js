...
/**
 * Determines if the provided uri is a valid file for Jove to import. Then
...
export function ipynbOpener(uri) {
  if (
    ...
    atom.config.get("jove-repl.importNotebookURI") === true
  ) {
    return _loadNotebook(uri, atom.config.get("jove-repl.importNotebookResults"));
  }
}

/**
 * Determines if the provided event is trying to open a valid file for Jove
 * to import. Otherwise it will ask the user to chose a valid file for Jove
 * to import. Then it loads the notebook.
 *
 * @param {Event} event - Lumine Event from clicking in a treeview.
 */
export async function importNotebook(event) {
  ...
  if (filenameFromTreeView && path.extname(filenameFromTreeView) === ".ipynb") {
    return _loadNotebook(filenameFromTreeView, atom.config.get("jove-repl.importNotebookResults"));
  }
  ...
  for (const filePath of filePaths) {
    ...
    _loadNotebook(filePath, atom.config.get("jove-repl.importNotebookResults"));
  }
}

/**
 * Reads the given notebook file and coverts it to a text editor format with
 * Jove cell breakpoints. Optionally after opening the notebook, it will
...
export async function _loadNotebook(filename, importResults = false) {
  ...
  try {
    ...
    if (data.nbformat < 4) {
      atom.notifications.addError("Only notebook version 4+ is supported");
      return;
    }
  ...
  nb.cellOrder.forEach((value) => {
    ...
    nbCells.push(cell);
    const hyCell = toJoveCodeBlock(cell, `${commentStartString} `);
...
/**
 * Converts notebook cells to Jove code blocks.
 *
 * @param {Cell} cell - Notebook cell to convert
 * @param {String} commentStartString - The comment syntax of the code language.
 * @returns {Object} - A Jove Code Block.
 */
function toJoveCodeBlock(cell, commentStartString) {
...
/**
 * Creates a Jove cell header
 *
 * @param {String} commentStartString - The comment syntax of the code language.
 * @param {String} keyword - The keyword relating to the cell type.
 * @returns {String} - A Jove Cell Header.
 */
